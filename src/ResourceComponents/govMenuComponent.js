...
class govMenuComponent extends React.Component{
    ...
    findAllResource(p){
        ResourceService.resourceMenuPaginated(5,p).then(res=>{
            this.setState({resources:res.data.content});
        });
        ResourceService.countByRestypeid(5).then(res=>{
            this.setState({totalElements:res.data});
        });
        ResourceService.pageByRestypeid(5).then(res=>{
    ...
    render(){
       return(
        <div>
        <br></br>
        <h3 className="text-center font-weight-bold text-secondary">能源平台政府侧菜单</h3>
        ...
        <div className="text-center color-font font-weight-bold">共{this.state.totalElements}资源(能源平台侧政府菜单)</div>
    </div>
       )
    }
}

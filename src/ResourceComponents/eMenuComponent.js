...
class eMenuComponent extends React.Component{
    ...
    findAllResource(p){
        ResourceService.resourceMenuPaginated(3,p).then(res=>{
            this.setState({resources:res.data.content});
        });
        ResourceService.countByRestypeid(3).then(res=>{
            this.setState({totalElements:res.data});
        });
        ResourceService.pageByRestypeid(3).then(res=>{
    ...
    render(){
       return(
        <div>
        <h3 className="text-center" style={{color:"#666669",marginTop:"3.5%"}}>能源平台菜单</h3>
        ...
        <div>共{this.state.totalElements}资源(能源平台菜单)</div>
        </div>
    </div>
       )
    }

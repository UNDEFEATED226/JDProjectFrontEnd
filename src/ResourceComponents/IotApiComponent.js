...
class IotApiResourceComponent extends React.Component{
    ...
    findAllResource(p){
        ResourceService.resourceMenuPaginated(2,p).then(res=>{
            this.setState({resources:res.data.content});
        });
        ResourceService.countByRestypeid(2).then(res=>{
            this.setState({totalElements:res.data});
        });
        ResourceService.pageByRestypeid(2).then(res=>{
    ...
    render(){
       return(
        <div>
        <br></br>
        <h3 className="text-center font-weight-bold text-secondary">物管平台API</h3>
        ...
        <table className="table f-size table-boarder" style={{color:"grey"}}> 
           <thead className="text-justify">
                <tr>
                  ...
                  <th className="yext-center" style={{columnWidth:"300px"}}>操作</th>
                </tr>
                </thead>
             <tbody>
                 {
        ...
        <div className="text-center color-font font-weight-bold">共{this.state.totalElements}资源(物管平台API)</div>
    </div>
       )
    }
}

export default IotApiResourceComponent 